...
export class InMemoryCheckInRepositories implements CheckInsRepository {
  ...
  async findManyByUserId(userId: string, page: number): Promise<CheckIn[]> {
    ...
    const firstTwentyCheckIns = checkIns?.slice((page - 1) * 20, page * 20)

    if (!firstTwentyCheckIns) return []

    return firstTwentyCheckIns
  }
